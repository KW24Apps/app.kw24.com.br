...
Credenciais vêm de `.dbconfig.json` (gerado por api/relatorio-conexao.php a partir
do painel "Configurar conexão" em Relatórios BI — kwconfig.relatorios_bi_conexoes)
nesta mesma pasta. Se o arquivo não existir (ex.: dev local, modo demo), cai de
volta para variáveis de ambiente (.env) — mesmo comportamento de antes.
Lido a cada conexão (não só no import) para que uma alteração salva no painel
valha sem precisar reiniciar o Gunicorn.
...
def _load_db_config():
    if os.path.exists(_CONFIG_FILE):
        ...
        return {
            ...
            "dbname":   cfg.get("dbname", "bx_sync_nimbus_tax"),
